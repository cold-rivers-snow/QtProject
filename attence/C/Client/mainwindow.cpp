...
//    date = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
    ...
    QString str = QString("select count(*) as row from work where user_id = '%1' and  work_date >='%2'  and work_date < '%3'").arg(qid).arg(ui->dateTimeEdit_2->text()).arg(ui->dateTimeEdit_3->text());
    query.exec(str);
    if(query.first())
    {
        ...
        if(nRow == 0)
        {
            QMessageBox::information(this ,tr("提示") , tr("查询的时间段有误!"));
            return;
        }
        setTabelH();
    ...
    QString sql = QString("select * from work where user_id = '%1' and  work_date >='%2'  and work_date < '%3'").arg(qid).arg(ui->dateTimeEdit_2->text()).arg(ui->dateTimeEdit_3->text());
    query.exec(sql);
